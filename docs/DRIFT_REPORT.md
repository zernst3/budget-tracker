...
### 6. Infrastructure wiring lives in the `budget-ui` server module
- **File:** `crates/budget-ui/src/server_state.rs` — the `from_connections` factory bodies on `AppState`, `MonthViewState`, `TriageState`.
- **Note (post-merge):** `budget-server` is no longer a separate crate. Per PORT-FULLSTACK-1 the fullstack app is ONE crate (`budget-ui`); the native entry binary `budget-server` (`src/main.rs`) + the router wiring (`src/server.rs`, `build_router()`) now live inside `budget-ui` itself, all `#[cfg(feature = "server")]`-gated. So "move the wiring out of budget-ui into budget-server" is no longer literally available — the drift below is now an *intra-crate* layering concern (where in budget-ui's server-side modules the infra-concretion wiring belongs), not a cross-crate move.
- **Problem:** `budget-ui`'s server-side code directly imports and instantiates concrete infra types (`PostgresMonthRepository`, `Argon2idHasher`, `AzureKeyVault`, `HttpPlaidApi`, `SeaOrmUowProvider`, etc.) and takes `sea_orm::DatabaseConnection` in public signatures. The canonical layering is `... ← infrastructure ← app_services ← server` (RUST-ENTITIES-13 / ARCH-STRICT-LAYERING-1).
- **Fix:** Keep the `from_connections` constructor *bodies* (the infra wiring + `DatabaseConnection` params) confined to the server-only modules (`server.rs` / `server_state.rs`), and keep the `new(...)` constructors that accept assembled `Arc<dyn Trait>` objects where they are — those are correct. The remaining cleanup is to stop exposing `sea_orm::DatabaseConnection` on the public type surface.
